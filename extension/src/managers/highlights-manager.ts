...
export async function exportHighlights(): Promise<void> {
	try {
		...
		const fileName = `sieve-clipper-highlights-${timestamp}.json`;
			...
			if (navigator.share) {
				try {
					await navigator.share({
						...
						title: 'Exported Sieve Clipper Highlights',
						text: 'Here are your exported highlights from Sieve Clipper.'
					});
				} catch (error) {
